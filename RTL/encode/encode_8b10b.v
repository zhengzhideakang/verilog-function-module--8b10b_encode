 ...
 * @LastEditTime : 2026-08-12 14:00:35
...
#(
  ...
  output reg       dout_10b_valid, // 输出有效, 高电平有效, 比din_8b_valid延时1个clk时钟周期
...
wire [4:0] din_5b_edcba = din_8b[4:0];
...
//++ 数据码3b/4b编码 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
wire [2:0] din_3b_hgf = din_8b[7:5]; // 高3位
...
always @(posedge clk) begin
  if (~rstn)
    current_rd <= 1'b0;
  else if (din_8b_valid
          && ((din_8b_is_k_or_d_n && ~k_code_10b_balance_flag)
                // 5b/6b与3b/4b都平衡或都不平衡, 则最终RD不反转; 否则, 反转
              || (~din_8b_is_k_or_d_n && d_code_6b_balance_flag != d_code_4b_balance_flag)
              )
          )
...
//++ 最终8b/10b编码输出 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
always @(posedge clk) begin
  if (din_8b_valid) // din_8b_valid 与 din_8b_is_k_or_d_n 是对齐的
    if (din_8b_is_k_or_d_n)
      dout_10b <= k_code_10b;
    else
...
if (ERROR_DETECT_EN) begin
  ...
  always @(posedge clk) begin
    if (~rstn)
      dout_k_error <= 1'b0;
    else if (din_8b_valid && din_8b_is_k_or_d_n && k_code_10b == 'd0)
      dout_k_error <= 1'b1;
    else
      dout_k_error <= 1'b0;
  end
